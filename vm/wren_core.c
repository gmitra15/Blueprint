...
DEF_PRIMITIVE(num_fixed)
{
  int32_t val = (int32_t)AS_NUM(args[0]);
  RETURN_NUM(val << SHIFT_AMOUNT);
}

DEF_PRIMITIVE(num_float)
{
  int32_t val = (int32_t)AS_NUM(args[0]);
  double div = 1 << SHIFT_AMOUNT;
  RETURN_NUM((val >> SHIFT_AMOUNT) + (val & SHIFT_MASK) / div);
}

DEF_PRIMITIVE(num_fixedAdd)
{
  if (!validateNum(vm, args[1], "Right operand")) return false;
  int32_t left = (int32_t)AS_NUM(args[0]);
  int32_t right = (int32_t)AS_NUM(args[1]);
  RETURN_NUM(left + right);
}

DEF_PRIMITIVE(num_fixedSub)
{
  if (!validateNum(vm, args[1], "Right operand")) return false;
  int32_t left = (int32_t)AS_NUM(args[0]);
  int32_t right = (int32_t)AS_NUM(args[1]);
...
DEF_PRIMITIVE(num_fixedMul)
{
  if (!validateNum(vm, args[1], "Right operand")) return false;
  int32_t left = (int32_t)AS_NUM(args[0]);
  int32_t right = (int32_t)AS_NUM(args[1]);
  RETURN_NUM((left * right) >> SHIFT_AMOUNT);
}

DEF_PRIMITIVE(num_fixedDiv)
{
  if (!validateNum(vm, args[1], "Right operand")) return false;
  int32_t left = (int32_t)AS_NUM(args[0]);
  int32_t right = (int32_t)AS_NUM(args[1]);
  RETURN_NUM((left << SHIFT_AMOUNT) / right);
}
...
void wrenInitializeCore(WrenVM* vm)
{
  ...
  PRIMITIVE(vm->numClass, "fixed", num_fixed);
  PRIMITIVE(vm->numClass, "float", num_float)
  PRIMITIVE(vm->numClass, "add(_)", num_fixedAdd);
  PRIMITIVE(vm->numClass, "sub(_)", num_fixedSub);
  PRIMITIVE(vm->numClass, "mul(_)", num_fixedMul);
  PRIMITIVE(vm->numClass, "div(_)", num_fixedDiv);
